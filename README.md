...
Firstly, public and private keys are generated at the controller. Secondly, the controller's public key has to be transferred to the managed hosts, letting them know about the controller and its public key. The second part requires a valid username and password to access the managed host. With the private and public keys in the right places, the controller can access the managed hosts using a key-based authentication.
