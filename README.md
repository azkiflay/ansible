...
While tools such as [Vagrant](https://developer.hashicorp.com/vagrant) and [Terraform](https://developer.hashicorp.com/terraform) are systems that orchestrate infrastructure such as virtual machines (VMs) and containers, Ansible manages the configuration of the infrastructure. Put another way, after server infrastructure has been created using IaC tools, Ansible is used to configure the servers. Ansible utilizes Yet Another Markup Language (YAML) to describe the desired state of infrastructure. As a result, Ansible has a *declarative* configuration style rather than *imperative*. In the case of the latter, the user is required to specify the exact details of the infrastructure, while a description of the infrastructre state suffices in Ansible.
