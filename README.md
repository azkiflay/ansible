...
Moreover, unlike other CM systems, Ansible is agentless, meaning it does not require a software agent to be installed on the managed hosts. This reduces security risks and administration costs of the managed hosts as Ansible can be installed only at a controller node. From the controller node, Ansible uses Secure Shell (SSH) to push configuration to managed hosts and make the desired changes accordingly. With SSH being the most secure method to connect to remote hosts, Ansible leverages SSH to configure a number of servers remotely.
